...
import { generateSessionTitle, isAutoGeneratedName } from "../utils/title-gen.js";
...
export class ConnectionDO implements DurableObject {
  ...
  private async handleBrowserMessage(
    ...
    if (msg.type === "user.message") {
      ...
      // Auto-generate session title on first user message
  ...
  /**
   * Auto-rename a session based on the first user message.
   * Only renames if the session has an auto-generated name ("Session N").
   */
  private async maybeAutoRenameSession(sessionKey: string, messageText: string): Promise<void> {
    try {
      // Get session info from database
      const session = await this.env.DB.prepare(
        "SELECT id, channel_id, name FROM sessions WHERE session_key = ?",
      )
        .bind(sessionKey)
        .first<{ id: string; channel_id: string; name: string }>();

      if (!session) {
        console.log(`[AutoRename] Session not found for key: ${sessionKey}`);
        return;
      }

      // Only rename if it has an auto-generated name
      if (!isAutoGeneratedName(session.name)) {
        console.log(`[AutoRename] Session already has custom name: "${session.name}"`);
        return;
      }

      // Count existing messages to ensure this is the first user message
      const countRow = await this.env.DB.prepare(
        "SELECT COUNT(*) as cnt FROM messages WHERE session_key = ? AND sender = 'user'",
      )
        .bind(sessionKey)
        .first<{ cnt: number }>();

      // If more than 1 user message, skip (session already has content)
      if ((countRow?.cnt ?? 0) > 1) {
        console.log(`[AutoRename] Session already has ${countRow?.cnt} user messages, skipping`);
        return;
      }

      // Generate title based on message content
      const newTitle = await generateSessionTitle(messageText, this.env);
      if (!newTitle) {
        console.log("[AutoRename] Failed to generate title, keeping default");
        return;
      }

      // Update session name in database
      await this.env.DB.prepare(
        "UPDATE sessions SET name = ?, updated_at = unixepoch() WHERE id = ?",
      )
        .bind(newTitle, session.id)
        .run();

      console.log(`[AutoRename] Renamed session "${session.name}" → "${newTitle}"`);

      // Broadcast rename event to all browser clients
      this.broadcastToBrowsers(
        JSON.stringify({
      ...
      console.error("[AutoRename] Error auto-renaming session:", err);
    }
  }
